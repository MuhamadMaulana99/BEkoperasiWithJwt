const { models: { pengajuan, loginModel, masterNasabah } } = require('../../model/index.js');
...
module.exports = {
  ...
  addPengajuan: async (req, res) => {
    const {
      id_users, id_mst_nasabah, penjualan, hargaPokok, biaya, labaUsaha, pendapatanLain, jumlahPendapatan,
      kebutuhanRumahTangga, biayaPendidikan, biayaLainya, jumlahBiayaLuarUsaha, pendapatanBersih,
      rasioAngsuran, jangkaWaktu, nominalPermohonan, tujuanPembiayaan, jaminan, accPermohonan,
      nomorAkad, status, statusBy, statusAt, foto
    } = req.body;
    ...
    try {
      ...
      res.status(500).json({ error: 'Error adding Pengajuan: ' + error.message });
    }
  },
  ...
  getPengajuan: async (req, res) => {
    try {
      ...
      const get = await pengajuan.findAll({
          ...
          attributes: [
            'id_pengajuans', 'id_users', 'id_mst_nasabah', 'penjualan', 'hargaPokok', 'biaya', 'labaUsaha',
            'pendapatanLain', 'jumlahPendapatan', 'kebutuhanRumahTangga', 'biayaPendidikan', 'biayaLainya',
            'jumlahBiayaLuarUsaha', 'pendapatanBersih', 'rasioAngsuran', 'jangkaWaktu', 'nominalPermohonan',
            'tujuanPembiayaan', 'jaminan', 'accPermohonan', 'nomorAkad', 'status', 'statusBy', 'statusAt', 'foto'
          ]
      });
  ...
  approvalPengajuan: async (req, res) => {
    ...
    try {
      ...
      const update = await pengajuan.update({ status }, {
        where: { id_pengajuans }
      });
      res.json(update);
    } catch (error) {
  ...
  getPengajuanByNoAkad: async (req, res) => {
    try {
      ...
      res.status(500).json({ error: 'Error fetching Pengajuan by No Akad: ' + error.message });
    }
  },
  ...
  putPengajuan: async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ error: 'Error updating Pengajuan: ' + error.message });
    }
  },
  ...
  deletePengajuan: async (req, res) => {
    ...
    try {
      ...
      res.json({ message: `Pengajuan with id ${id_pengajuans} deleted successfully.` });
    } catch (error) {
      res.status(500).json({ error: 'Error deleting Pengajuan: ' + error.message });
    }
  }
};
